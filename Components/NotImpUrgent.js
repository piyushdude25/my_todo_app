...
export const NotImpUrgent = () => {
  ...
  const handleSubmit = async () => {
    try {
      const data = {
        not_imp_urgent: [{task: task}],
      };

      if (myIdData && myIdData._id) {
        const response = await axios.post(
          `https://myserver-ylrk.onrender.com/api/mytodo/not_imp_urgent/${myIdData._id}`,
          data,
          {
            headers: {
    ...
    try {
      if (myIdData && myIdData._id) {
        ...
        if (response.status === 200) {
          ...
          setUserData(userData.not_imp_urgent);
        } else {
    ...
    try {
      if (myIdData && myIdData._id) {
        const response = await axios.delete(
          `https://myserver-ylrk.onrender.com/api/mytodo/not_imp_urgent/${myIdData._id}/delete/${taskId}`,
        );
