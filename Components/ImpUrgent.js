...
export const ImpUrgent = () => {
  ...
  const handleSubmit = async () => {
    try {
      const data = {
        imp_urgent: [{task: task}],
      };

      if (myIdData && myIdData._id) {
        const response = await axios.post(
          `https://myserver-ylrk.onrender.com/api/mytodo/imp_urgent/${myIdData._id}`,
          data,
          {
            headers: {
    ...
    try {
      if (myIdData && myIdData._id) {
        ...
        if (response.status === 200) {
          ...
          setUserData(userData.imp_urgent);
        } else {
    ...
    try {
      if (myIdData && myIdData._id) {
        const response = await axios.delete(
          `https://myserver-ylrk.onrender.com/api/mytodo/imp_urgent/${myIdData._id}/delete/${taskId}`,
        );
    ...
    <ImageBackground
      ...
      >
        <View style={styles.container}>
          <Text style={{fontSize: 20, textAlign: 'center',marginBottom:5, color:"red"}}>
